import { getColdEvaluation } from "@/app/api/settings/actions/getColdEvaluation";
...
export default function ColdEvaluation() {
  ...
  const handleSubmit = () => {
    ...
    startTransition(() => {
      updateQcm(questions, departement_id, "cold_evaluation").then((data) => {
        if (data?.error) {
  ...
  useEffect(() => {
    ...
    startTransition(() => {
      getColdEvaluation(departement_id).then((data: any) => {
        if (data?.success) {
          const qcmData = data.success.cold_evaluation ?? [];
