...
import { getQcm } from "@/app/api/settings/actions/getQcm";
...
export default function Qcm(props: { evaluationType: string }) {
  ...
  const handleSubmit = () => {
    ...
    startTransition(() => {
      updateQcm(questions, departement_id, props.evaluationType).then(
        (data) => {
          if (data?.error) {
  ...
  useEffect(() => {
    ...
    startTransition(() => {
      getQcm(departement_id, props.evaluationType).then((data: any) => {
        if (data?.success) {
          console.log("data", data);
          const qcmData = data.success[props.evaluationType] ?? [];
  ...
  }, [departement_id, props.evaluationType]);
